  ...
  return (
    ...
    <div className=" w-[70%] m-auto mt-14 bg-black/50 p-3 rounded-lg">
      ...
      <div className="flex justify-between items-center mr-4 ml-4 font-mono mb-6">
        <h3 className="text-white p-5 bg-white/50 rounded-3xl  text-xl font-bold">Balance: ${Balance}</h3>
        <button
      ...
      >
        ...
        <div className="flex flex-row items-center justify-center" onChange={setOption}>
          <input type="radio" value='Income' name="option"></input>
          <span className="text-green-600 mr-4 ml-1 font-bold text-lg">Income</span>
          <input type="radio" value='Expense' name="option"></input>
          <span className="text-red-600 ml-1 font-bold text-lg">Expense</span>
        </div>
        <button
          className="p-3 bg-black mt-5 text-[14px] rounded-full font-bold border-solid border-[2px] text-white w-[80px] h-[50px] hover:bg-green-600"
      ...
      <div>
        <ul>
          {items.map((item, index) => (
            <li key={index + 1} className={`m-2 bg-blue-950/100 text-white capitalize rounded-md p-2 font-serif text-center text-lg`}>
                {item}</li>
          ))}
        </ul>
      </div>
